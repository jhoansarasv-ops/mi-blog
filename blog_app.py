...
total_comentarios = sum(len(p.get("comentarios", [])) for p in posts)
...
col3.metric("❤️ Likes", total_likes)
col4.metric("💬 Comentarios", total_comentarios)
col5.metric("🏷️ Categorías", categorias_unicas)
...
for i, post in enumerate(posts):
    ...
    with st.container():
        ...
        with col1:
            if st.button(f"📖 Leer", key=f"leer_{post['id']}", use_container_width=True):
        ...
        with col3:
            if st.button(f"💬 {num_comentarios}", key=f"coment_{post['id']}", use_container_width=True):
                st.session_state.ver_comentarios = post["id"]
                st.rerun()
        ...
        if st.session_state.get("ver_post") == post["id"]:
            with st.expander("📄 Artículo completo", expanded=True):
                ...
                if st.button("✖️ Cerrar artículo", key=f"cerrar_{post['id']}"):
                    st.session_state.ver_post = None
                    st.rerun()
        
        # COMENTARIOS
        if st.session_state.get("ver_comentarios") == post["id"]:
            with st.expander("💬 Comentarios", expanded=True):
                st.markdown(f"### 💬 Comentarios en: {post['titulo']}")
                
                comentarios = post.get("comentarios", [])
                
                if comentarios:
                    st.write(f"**{len(comentarios)} comentarios:**")
                    for j, comentario in enumerate(comentarios):
                        with st.container():
                            col1, col2 = st.columns([1, 4])
                            with col1:
                                st.markdown(f"👤 **{comentario['autor']}**")
                                st.caption(f"📅 {comentario['fecha']}")
                            with col2:
                                st.info(comentario['texto'])
                        st.divider()
                else:
                    st.info("📝 Aún no hay comentarios. ¡Sé el primero!")
                
                st.markdown("#### ✍️ Añadir comentario")
                with st.form(key=f"form_comentario_{post['id']}"):
                    nombre_comentario = st.text_input("👤 Tu nombre", placeholder="Anónimo")
                    texto_comentario = st.text_area("💬 Tu comentario", placeholder="Escribe tu opinión...", height=80)
                    
                    col1, col2 = st.columns(2)
                    with col1:
                        enviar = st.form_submit_button("📤 Enviar comentario", use_container_width=True)
                    with col2:
                        cerrar = st.form_submit_button("❌ Cerrar", use_container_width=True)
                    
                    if enviar and texto_comentario:
                        posts_data = cargar()
                        for p in posts_data:
                            if p["id"] == post["id"]:
                                ...
                                p["comentarios"].append({
                                    "autor": nombre_comentario or "Anónimo",
                    ...
                    if cerrar:
                        st.session_state.ver_comentarios = None
                        st.rerun()
...
st.divider()
st.markdown("### 🚀 Mi Blog")
st.caption("Compartiendo conocimiento desde 2026 | Hecho con ❤️ usando Streamlit + Python | 💬 Comentarios habilitados")
